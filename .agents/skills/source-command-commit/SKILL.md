...
승인된 본작업에서 Wiki, Agent 지침, Wiki 검사 도구만 변경되고 `wiki-maintainer` 검증과 기준점 갱신이 끝난 경우에는 별도 커밋 승인 없이 이번 작업 파일만 stage하여 커밋한다. 제품 기능 코드가 섞이면 이 예외를 적용하지 않는다.
...
일반 코드에서는 stage 또는 commit을 실행하지 않고 사용자 승인을 기다린다. Wiki 전용 자동 Git 조건이 충족되면 이 대기를 생략한다.

## 승인 후 또는 Wiki 예외 확인 후
