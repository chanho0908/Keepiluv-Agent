...
- 즉시 공식화할 근거가 부족한 재사용 지식은 `wiki/inbox`의 `knowledge-candidate`로 기록합니다. 검색·열람은 사용으로 세지 않고, 서로 다른 작업의 계획·구현·테스트·리뷰 판단에 실제 사용한 경우에만 작업당 한 번 근거를 누적합니다.
- 후보는 canonical 근거로 단독 사용하지 않습니다. 제품 정책·사용자 흐름·데이터 안전 규칙은 횟수와 무관하게, 일반 후보는 독립 작업 2회 이상 사용됐을 때 승격을 검토하되 자동 승격하지 않습니다.
