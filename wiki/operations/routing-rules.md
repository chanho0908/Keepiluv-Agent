...
- 명확한 구현 요청은 구현 승인으로 간주하고, 계획이 필요한 작업은 계획 승인 후 구현
...
- 구현 후: 일반 코드는 사용자 승인 전 커밋 금지. 승인된 Wiki 전용 작업은 검증 후 자동 Git 예외 적용
...
- `explore`, `writer`: 코드 표현을 새로운 도메인 용어로 번역하지 않고 공식 용어로 설명한다.
...
- 사용자의 명확한 `구현해줘`, `수정해줘`, `추가해줘` 요청은 해당 범위의 구현 승인이다.
- 계획이 필요한 작업은 계획을 먼저 제시하고, 사용자가 계획을 승인하면 테스트와 구현을 진행한다.
- 목표나 범위가 모호하거나, 파괴적 변경이 포함되거나, 영향 범위가 큰 요청만 별도 확인한다.
- 구현 승인은 일반 코드의 커밋, push, PR 승인을 포함하지 않는다. 승인된 Wiki 전용 작업은 별도 자동 Git 예외를 따른다.
...
- 승인된 본작업에서 Wiki, Agent 지침, Wiki 검사 도구만 변경되고 검증과 기준점 갱신이 끝난 경우에는 별도 커밋 승인 없이 이번 작업 파일만 커밋한다
- 제품 기능 코드가 섞이면 Wiki 전용 예외를 적용하지 않는다
...
- Wiki 전용 자동 흐름은 별도 승인 없이 push하고 항상 Draft PR을 생성한다
- 사용자가 Draft PR에 남긴 리뷰를 AI가 확인해 승인 범위 안에서 수정·답변하고 같은 PR에 반영한다
- AI는 PR을 자동 병합하지 않으며 최종 승인과 병합은 사용자가 수행한다
...
`wiki-maintainer`는 승인된 본작업에서 상태 탐지, 영향 분석, Wiki 갱신, validator 검사, Wiki 검증 기준점 반영을 별도 Wiki 승인 없이 수행합니다. 본작업과 무관한 도메인/운영 정책 변경이나 새로운 의미 결정은 사용자 승인을 받으며, Wiki 명령은 `Keepiluv-Agent` 저장소 루트에서 실행합니다.
