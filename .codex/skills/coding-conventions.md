...
구현 전 계획 단계에서는 먼저 사람이 이해할 수 있는 작업 흐름을 작성하고, 코드 레벨 세부사항은 기술 메모나 handoff로 분리한다.

계획 본문에 먼저 명시:

- [ ] **목표**: 무엇을 왜 바꾸는지
- [ ] **사용자 흐름**: 사용자가 어떤 순서로 경험하는지
- [ ] **정책/판단 기준**: 어떤 조건에서 어떤 동작을 하는지
- [ ] **예외 처리**: 실패, 취소, 재시도, 빈 상태 등
- [ ] **검증 계획**: 자동 테스트와 수동 확인 항목

기술 메모 또는 handoff에 분리:
