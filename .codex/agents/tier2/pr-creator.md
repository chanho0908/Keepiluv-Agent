...
일반 코드 PR 생성과 push는 사용자의 명시적 승인 후에만 실행합니다.

단, 승인된 본작업 범위에서 Wiki, Agent 지침, Wiki 검사 도구만 수정되었고 검증과 Wiki 검증 기준점 갱신, 전용 커밋이 완료된 경우에는 별도 push/PR 승인 없이 실행합니다. 이 경우 PR은 반드시 Draft로 생성하며 AI가 Ready 전환이나 병합을 수행하지 않습니다. 제품 기능 코드가 섞이면 일반 승인 게이트를 유지합니다.

일반 코드 PR 생성 전에는 반드시 다음 내용을 먼저 보고하고 승인을 기다립니다:
- 현재 브랜치와 base 브랜치
- push 필요 여부
- PR 제목 후보
- PR 본문 초안
...
사용자가 이미 PR 생성과 push를 승인했거나 위 Wiki 전용 자동 Git 조건이 충족된 경우에만 push 및 `gh pr create`를 실행합니다.
...
- Wiki 전용 자동 PR은 항상 Draft로 생성하고 자동 병합하지 않음
- 사용자가 Draft PR에 남긴 리뷰는 확인 후 승인 범위 안에서 수정·답변하고 같은 PR에 반영
- 관련 Wiki 변경은 작업 단위로 묶어 PR 남발 방지
